...
function DropDownSearch(props){
  ...
  function handleRemoveItem(item, event){
  ...
  function showDropdownOption(event){
    settoggleOption(true);
    event.stopPropagation()
  }

  function hideDropdownOption(event){
  ...
  return(
    <div onClick={hideDropdownOption} className="flex-1 h-screen ">
      <div  className="flex flex-col w-2/4 mx-auto px-3 py-3 ">
                ...
                {selectedOptions.map((item,index)=>{
                  return(
                    <div key={index} className="flex flex-row bg-green-100 mr-1 h-9 border-2 px-1 py-1 border-green-300 rounded ">
                      ...
                      <div onClick={(event)=>handleRemoveItem(item, event)} className="flex flex-col w-6 justify-center items-center">
                        <BsDashSquareFill />
                      </div>
                    </div>
                  )
